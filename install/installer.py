#Imports necesarios para el sistema funcional
import os
import time

#Aviso principal de los requisitos esperados
print("Instalador del cuentamuertes multijuego basado en web")
print("Vas a instalar el sistema de cuentamuertes. Este instalador asume: ")
...
#Preguntar si se quiere Instalar el cuentamuertes de cero o actualizar una versión ya instalada
upgrade = input("Quieres instalar una instancia nueva [i], o actualizar una existente [a]?")
while upgrade not in ["i", "a", "I", "A"]:
	upgrade = input("Opción no válda. Quieres instalar una instancia nueva [i], o actualizar una existente [a]?")

if upgrade in ["i", "I"]: #Opción de instalación completa
	#Si se quiere continuar, empieza el script de instalación
	...
	pageName = input("Nombre de quién lleva el contador, ejemplo escribir 'María' para establecer en la web 'Cuentamuertes de María': ")
	...
	replacevHost = "sed -i 's/    ServerName REPLACE/    ServerName " + vhostDomain + "/g' source/cuentamuertes.conf"
	os.system(replacevHost)
	
	#Replace en el script sql
	replaceSQL = "sed -i 's/ALTER TABLE public.juegos OWNER TO REPLACEONINSTALL;/ALTER TABLE public.juegos OWNER TO " + dbUser + ";/g' source/db.sql"
	os.system(replaceSQL)
	replaceSQL = "sed -i 's/ALTER TABLE public.main OWNER TO REPLACEONINSTALL;/ALTER TABLE public.main OWNER TO " + dbUser + ";/g' source/db.sql"
	os.system(replaceSQL)
	replaceSQL = "sed -i 's/ALTER TABLE public.main_id_seq OWNER TO REPLACEONINSTALL;/ALTER TABLE public.main_id_seq OWNER TO " + dbUser + ";/g' source/db.sql"
	os.system(replaceSQL)
	...
	cursor.close()
	conn.close()
	print("Se han creado los elementos de bases de datos! Ahora lo moveremos todo a su sitio y podrás crear tu VirtualiHost de Apache2 con webroot en /var/www/cuentamuertes/frontend/, iniciiarlo, y ya estará funcional.")
	time.sleep(2)
	
	#Mover todo a su lugar y cambiar los permisos a los correctos
	os.system("mkdir /var/www/cuentamuertes")
	os.system("mv source/apache/* /var/www/cuentamuertes/")
	os.system("chown root:www-data -R /var/www/cuentamuertes/frontend/")
	os.system("mv source/cuentamuertes.conf /etc/apache2/sites-available/")
	os.system("a2ensite cuentamuertes")
	print("Todo listo! Recargando apache2 para que el site esté activo...")
	time.sleep(2)
	os.system("service apache2 reload")
	print("Listo! Que disfrutes del cuentamuertes! ya puedes borrar esta carpeta de instalación si quieres")
else: #Opción de actualización del frontend
	print("Has elegido actualizar la instancia ya instalada! Un momento...")

	#Extraer a su carpeta los archivos de la webApp
	os.system("mkdir source/apache")
	os.system("tar -xvzf source/files.tar.gz -C source/apache")

	#Borrar el frontend anterior
	os.system("rm -r /var/www/cuentamuertes/frontend/*")

	#Mover a su sitio el nuevo frontend
	os.system("mv source/apache/frontend/* /var/www/cuentamuertes/frontend")
	os.system("chown root:www-data -R /var/www/cuentamuertes/frontend/")
	print("Todo listo! Recargando apache2 para que el site esté activo...")
	time.sleep(2)

	#Recargar Apache2 con los nuevos permisos y archivos
	os.system("service apache2 reload")
	print("Listo! Que disfrutes del cuentamuertes actualizado! ya puedes borrar esta carpeta de instalación si quieres")
